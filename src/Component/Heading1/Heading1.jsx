...
export default function Heading1({ headingText }) {
  ...
  return (
    <div className={style.Heading1+" d-flex justify-content-center align-items-center position-relative"}>
      {images.map((image)=>(
        <img key={image.id} src={image.src} alt={image.alt} className={style.image}/>
      ))}
      <h2>{headingText}</h2>
    </div>
  );
